...
realm_chs = pysaintcoinach.ARealmReversed(game_path, pysaintcoinach.Language.chinese_simplified)  # 国服
realm = pysaintcoinach.ARealmReversed(game_path_eng, pysaintcoinach.Language.english)  # 国际服
...
for class_job in realm.game_data.get_sheet('ClassJob'):
    if class_job.key and class_job['Abbreviation'] and class_job['ClassJobCategory'].key in {30, 31} and not class_job['StartingTown'].key:
        ...
        with open(f'{to_under_line(class_job["Name"])}.py', 'w+', encoding='utf-8') as f:
...
""")
            for action in action_sheet:
                if (
                    ...
                    f.write(f"""
    class {to_hump(action['Name'])}(ActionBase):
        \"""
        {desc}""")
                    for status in status_by_name.get(action['Name'], []):
                        f.write(f"\n        {status.key}, {status['Name']}, {status['Name']}, {line_break.sub(' ', status['Description'])}")
                    try:
                        f.write(f"""
        \"""
        id = {action.key}
        name = {{"{action['Name']}", "{action_sheet_chs[action.key]['Name']}"}}
""")
                    except:
                        f.write(f"""
        \"""
