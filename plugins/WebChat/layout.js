    ...
    setup(props) {
        ...
        const stop_server = () => {
            const loading = element.ElLoading.service({
                lock: true, text: '关闭WebChat 服务器中...',
                spinner: 'el-icon-loading', background: 'rgba(255,255,255,0.71)'
            })
            plugin.value.run_single('layout_stop_server').then(
                _ => element.ElNotification.success({title: `WebChat 服务器关闭成功`}),
                err => element.ElNotification.error({title: `WebChat 服务器关闭失败`, message: err.data}),
        ...
        const start_server = () => {
            ...
            plugin.value.run_single('layout_start_server').then(
                _ => {
                    element.ElNotification.success({title: `WebChat 服务器启动中`});
                    work.value = true
                },
                err => element.ElNotification.error({title: `WebChat 服务器启动失败`, message: err.data}),
